...
import { IS_SELECT_FORM_MODEL } from "./self-test-records-form.model";//引入是否全选权限下拉表单控件
import { ROLE_MANAGE_FORM_LAYOUT } from "./self-test-records-form.layout";//引入表单布局

@Component({
  selector: 'app-self-test-records',
  templateUrl: './self-test-records.component.html',
  styleUrls: ['./self-test-records.component.scss']
})
export class SelfTestRecordsComponent implements OnInit {
  ...
  private rolesUrl = `${AppSettings.env_vars.API_URL}/user-manage/role-manage`;//用来获取角色的接口
