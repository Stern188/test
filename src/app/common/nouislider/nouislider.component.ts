...
export class NouisliderComponent implements ControlValueAccessor, OnInit, OnChanges {
  ...
  ngOnInit(): void {
    let inputsConfig = JSON.parse(JSON.stringify({
      ...
      limit: this.limit,
      start: this.value? this.value:(this.formControl !== undefined ? this.formControl.value : this.ngModel),
      step: this.step,
    ...
    this.slider = noUiSlider.create(
      this.el.nativeElement.querySelector('div.slider') ,
    ...
    this.slider.on('set', (values: string[], handle: number, unencoded: number[]) => {
      ...
      if(Array.isArray(v) && this._value[handle] != v[handle]) {
        emitEvents = true;
      }
      if(!Array.isArray(v) && this._value != v) {
        emitEvents = true;
      }
      if(emitEvents) {
        this.set.emit(v);
