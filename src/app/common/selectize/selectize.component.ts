...
export class SelectizeComponent implements OnInit,  DoCheck, ControlValueAccessor {
  ...
  reset() {
    ...
    this._oldOptions = ld.cloneDeep(this.options);
    this._oldOptionGroups = ld.cloneDeep(this.optionGroups);
   ...
   */
  ngDoCheck(): void {
    if (!ld.isEqual(this._oldOptions, this.options)) {
      this.onSelectizeOptionsChange();
      this._oldOptions = ld.cloneDeep(this.options);
    }
    if (!ld.isEqual(this._oldOptionGroups, this.optionGroups)) {
      this.onSelectizeOptionGroupChange();
      this._oldOptionGroups = ld.cloneDeep(this.optionGroups);
    }
    this.evalHasError();
  ...
  onSelectizeOptionsChange(): void {
    const optionsRemoved = ld.differenceWith(this._oldOptions, this.options, (oldValue:any, newValue:any) => {
    ...
    const newOptionsAdded = ld.differenceWith(this.options, this._oldOptions, (oldValue:any, newValue:any) => {
      return oldValue[this.selectize.settings.valueField] === newValue[this.selectize.settings.valueField]
        && oldValue[this.selectize.settings.labelField] === newValue[this.selectize.settings.labelField];
    });
    ...
    if (newOptionsAdded && newOptionsAdded.length > 0) {
      newOptionsAdded.forEach((option:any) => {
        this.selectize.addOption(ld.cloneDeep(option));
      });
    }
    this.updatePlaceholder();
  }
  ...
  set value(value: string[]) {
    if (this._value !== value) {
      setTimeout(() => { // Fix for change after check issue in development mode.
        this._value = ld.cloneDeep(value);
      });
    }
  }
}
